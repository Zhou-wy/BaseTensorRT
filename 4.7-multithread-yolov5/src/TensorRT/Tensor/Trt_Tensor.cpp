 ...
 * @LastEditors: zwy
 * @LastEditTime: 2022-10-11 19:39:39
 */
#include <algorithm>
...
namespace TRT
{
    ...
    int data_type_size(DataType dt)
    {
        ...
        default:
        {
            INFOE("Not support data type: %d", dt);
            return -1;
        }
        }
    }
    ...
    Tensor &Tensor::copy_from_gpu(size_t offset, const void *src, size_t num_element, int device_id)
    {
        ...
        {
            INFOE("could not support head type %d", head_);
        }
        return *this;
    }
    ...
    void Tensor::reference_data(const std::vector<int> &shape, void *cpu_data, size_t cpu_size, void *gpu_data,
                                size_t gpu_size, DataType dtype)
    {
        dtype_ = dtype;
