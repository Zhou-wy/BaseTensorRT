 ...
 * @version:
 * @Author: zwy
 * @Date: 2022-10-12 16:09:14
 * @LastEditors: zwy
 * @LastEditTime: 2022-10-12 19:08:35
 */
...
#include "TensorRT/utils/cuda_tools.cuh"
#include "TensorRT/utils/simple_logger.hpp"
...
namespace YoloV5
{
    ...
    struct Job
    {
        std::shared_ptr<std::promise<BoxArray>> pro;
        cv::Mat input;
        float d2i[6];
    };
    ...
    public:
        ...
        bool startup(const std::string &file, int gpuid, float confidence_threshold, float nms_threshold)
        {
        ...
        virtual std::shared_future<BoxArray> commit(const cv::Mat &image) override
        {
            ...
            if (image.empty())
            {
                ...
                return std::shared_future<BoxArray>();
            }

            Job job;
            job.pro.reset(new std::promise<BoxArray>());
            ...
            i2d[0] = scale;
            i2d[1] = 0;
            i2d[2] = (-scale * image.cols + input_width_ + scale - 1) * 0.5;
            i2d[3] = 0;
            i2d[4] = scale;
            i2d[5] = (-scale * image.rows + input_height_ + scale - 1) * 0.5;
            ...
            std::shared_future<BoxArray> fut = job.pro->get_future();
            {
    ...
    std::shared_ptr<Infer> create_infer(const std::string &file, int gpuid, float confidence_threshold, float nms_threshold)
    {
