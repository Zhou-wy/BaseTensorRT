...
public:
    ...
    bool InitScale()
    {
        ...
        if (!swsContext)
        {
            INFOE("初始化格式转换上下文sws_getCachedContext失败");
            return false;
        }
        ...
        if (ret != 0)
        {
            INFOE("av_frame_get_buffer失败");
            return false;
        }
        INFO("XMediaEncode InitScale 完成");
        return true;
    }
    ...
    bool InitVideoCodec()
    {
        ...
        {
            INFOE("InitVideoCodec 获取解码器上下文出错");
            return false;
        }
        // 配置编码器参数
        ...
        INFO("InitVideoCodec 获取CUP NUM: %d ", videoCodecContext->thread_count);
        //压缩后每秒视频的bit位大小 50kB
        ...
        // videoCodecContext->sample_fmt = AV_SAMPLE_FMT_FLT;

        ret = avcodec_open2(videoCodecContext, 0, 0);
        if (ret != 0)
        {
            INFOE("avcodec_open2出错");
            return false;
        }

        INFO("XMediaEncode InitVideoCodec 完成");
        return true;
    }
    ...
    bool InitResample()
    {
        ...
        if (!swrContext)
        {
            INFOE("swrContext failed!");
            return false;
        }
        int ret = swr_init(swrContext);
        if (ret != 0)
        {
            INFOE("swr_init failed!");
            return false;
        }
        ...
        if (ret != 0)
        {
            INFOE("av_frame_get_buffer failed!");
            return false;
        }
